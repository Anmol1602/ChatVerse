...
export async function handler(event, context) {
  ...
  try {
    console.log('Starting simplified files table migration...');

    // Create files table without foreign key constraints first
    await sql`
    ...
    return {
      ...
      headers,
      body: JSON.stringify({ 
        message: 'Simplified files table migration completed successfully', 
        timestamp: new Date().toISOString() 
      })
    };
  } catch (error) {
    console.error('Simplified files table migration failed:', error);
    return {
      statusCode: 500,
      headers,
      body: JSON.stringify({ 
        error: 'Simplified files table migration failed', 
